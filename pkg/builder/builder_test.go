...
type recordingObserver[S any, E any, SP any, EP any] struct {
	calls int
}

func (o *recordingObserver[S, E, SP, EP]) OnStep(_ S, _ SP, _ S, _ E, _ EP) {
	o.calls++
}
...
func TestDFABuilderWithObserverOverride(t *testing.T) {
	...
	obs := &recordingObserver[int, string, struct{}, struct{}]{}
	b.WithObserver(obs)
...
func TestNFABuilderWithNFAOverride(t *testing.T) {
	...
	override := nfa.New(nfa.Config[int, string]{
		States:    []int{0, 1},
		Alphabet:  []string{"a"},
		Start:     0,
		Accepting: []int{1},
...
func TestNFABuilderBuildEngineObserverOrder(t *testing.T) {
	...
	var order []string
	b.WithOnEnterAny(func(from *set.Set[int], _ struct{}, to *set.Set[int], _ string, _ struct{}) {
		order = append(order, "enter:any")
	})
	b.WithOnExitAny(func(from *set.Set[int], _ struct{}, to *set.Set[int], _ string, _ struct{}) {
		order = append(order, "exit:any")
	})
...
func TestNFABuilderWithObserverOverride(t *testing.T) {
	...
	obs := &recordingObserver[*set.Set[int], string, struct{}, struct{}]{}
	b.WithObserver(obs)
