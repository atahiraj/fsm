...
func (n *NFA[S, A]) DeltaStar(s S, w []A) []S {
	return n.DeltaStarSet(set.New(s), w).Slice()
}

// DeltaSet applies δ : 𝒫(Q) × Σ → 𝒫(Q) to a set of states.
func (n *NFA[S, A]) DeltaSet(Q *set.Set[S], a A) *set.Set[S] {
	out := set.New[S]()
	if Q == nil || Q.IsEmpty() {
		return out
	}
	Q.Iter()(func(s S) bool {
		out.Add(n.Delta(s, a)...)
		return true
	})
	return out
...
func (n *NFA[S, A]) DeltaStarSet(Q *set.Set[S], w []A) *set.Set[S] {
	cur := n.EpsilonClosureSet(Q)
	for _, a := range w {
...
func (n *NFA[S, A]) EpsilonClosure(s S) []S {
	return n.EpsilonClosureSet(set.New(s)).Slice()
}

// EpsilonSet applies δ to a set of states on ε.
func (n *NFA[S, A]) EpsilonSet(Q *set.Set[S]) *set.Set[S] {
	out := set.New[S]()
	if Q == nil || Q.IsEmpty() {
		return out
	}
...
func (n *NFA[S, A]) EpsilonClosureSet(Q *set.Set[S]) *set.Set[S] {
	out := set.New[S]()
	if Q == nil || Q.IsEmpty() {
		return out
	}
	stack := make([]S, 0, Q.Len())
	Q.Iter()(func(s S) bool {
		if !out.Has(s) {
			out.Add(s)
...
func (n *NFA[S, A]) StartSet() *set.Set[S] {
	return set.New(n.start)
}
...
func (n *NFA[S, A]) IsAccepting(s *set.Set[S]) bool {
	return n.accepting.Intersects(s)
}
